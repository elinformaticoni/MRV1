...
|---|---|
| Versión | MR V1.9 |
| Nombre del paquete | `MRV1.9` — el paquete (carpeta/ZIP) y esta guía llevan siempre el mismo nombre y la misma versión, esquema `MRV1.<sub>` (igual que la carpeta de instalación `C:\ProgramData\MRV1`, ver 3.4); la guía viaja **dentro** del paquete, no se entrega suelta. |
...
C:\ProgramData\MRV1\
    ...
    diag\             monitor.log, ftp_worker.log, db_worker.log, db_sync_state.json (con rotación por tamaño)
    status.json
    ftp_status.json
    db_status.json
```
...
## 14. Archivos del paquete (versión 1.9)
